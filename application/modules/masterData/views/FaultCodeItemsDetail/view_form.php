    		...
    		<div class="col-sm-3">
                <select class="form-control select" id="category" name="category" required>
                    ...
                    <?php foreach($data_cat as $row): ?>
                        <option value="<?php echo $row['fCode'];?>" <?php if($row['fCode'] == @$data_row['fType']){echo 'selected';}  ?> ><?php echo $row['fName'];?></option>
                    <?php endforeach; ?>
                </select>
    		</div>
    	</div>
...
<script type="text/javascript">
	...
	form.find('[name="code"]').on('blur', function() {
		...
		$.ajax({
			url: '<?php echo site_url('/masterData/FaultCodeItemsDetail/is_code_exist'); ?>',
			type: 'post',
			dataType: 'json',
