        ...
        <div class="form-group">
    		...
    		<div class="col-sm-3">
                <select name="type" class="form-control select" required>
					<option value=""></option>
					<option value="1" <?php if(isset($data_row['fType'])){ echo ($data_row['fType'] == 1) ? 'selected':'';}?>>Damage</option>
					<option value="2" <?php if(isset($data_row['fType'])){ echo ($data_row['fType'] == 2) ? 'selected':'';}?>>Cleanliness</option>
				</select>
    		</div>
    	</div>
		<div class="form-group">
...
<script type="text/javascript">
	...
	form.find('[name="code"]').on('blur', function() {
		...
		$.ajax({
			url: '<?php echo site_url('/masterData/FaultTypes/is_code_exist'); ?>',
			type: 'post',
			dataType: 'json',
